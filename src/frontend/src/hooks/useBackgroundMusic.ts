...
export function useBackgroundMusic(
  ...
  const getDieHardNotes = useCallback((): Array<{
    freq: number;
    duration: number;
    type: OscillatorType;
  }> => {
  ...
  const getBondNotes = useCallback((): Array<{
    freq: number;
    duration: number;
    type: OscillatorType;
  }> => {
  ...
  useEffect(() => {
    ...
    const _newTrack: TrackName = hasEnemies ? "diehard" : "diehard"; // Die Hard is always default
    // Bond plays during calm moments
    const targetTrack: TrackName =
      gameActive && hasEnemies ? "diehard" : gameActive ? "bond" : "diehard";
