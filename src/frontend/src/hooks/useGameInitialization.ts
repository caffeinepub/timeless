...
export function useGameInitialization(): GameInitializationState {
  ...
  const _startStageTimer = useCallback(
    (stageName: string, nextAction: () => void) => {
      clearStageTimer();
      stageTimerRef.current = setTimeout(() => {
        ...
        if (retryCount < MAX_RETRIES) {
          ...
          failWithError(
            `Initialization failed at stage "${stageName}" after ${MAX_RETRIES + 1} attempts.`,
          );
        }
      }, STAGE_TIMEOUT_MS);
  ...
  const markStageComplete = useCallback(
    (
      ...
      const allDone = stageOrder.every((s) =>
        stagesCompletedRef.current.has(s),
      );
  ...
  const retry = useCallback(() => {
    if (retryCount >= MAX_RETRIES) {
      setErrorMessage(
        `Maximum retry attempts (${MAX_RETRIES}) reached. Please reload the page.`,
      );
      return;
    }
